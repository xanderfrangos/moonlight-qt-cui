...
#define SER_GAMEPADMENUTRIGGER "gamepadmenutrigger"
...
void StreamingPreferences::reload()
{
    ...
    if (settings.contains(SER_DEBANDMODE)) {
        bool validMode = false;
        const int savedMode = settings.value(SER_DEBANDMODE).toInt(&validMode);
        if (validMode && savedMode >= DB_OFF && savedMode <= DB_STRONG) {
    ...
    performanceOverlayMode = POM_TEXT_AND_GRAPHS;
    if (settings.contains(SER_PERFOVERLAYMODE)) {
        bool validMode = false;
        const int savedMode = settings.value(SER_PERFOVERLAYMODE).toInt(&validMode);
    ...
    gamepadMenuTrigger = static_cast<GamepadMenuTrigger>(settings.value(SER_GAMEPADMENUTRIGGER,
                                                         static_cast<int>(GamepadMenuTrigger::GMT_COMBO)).toInt());
...
void StreamingPreferences::save()
{
    ...
    settings.setValue(SER_GAMEPADMENUTRIGGER, gamepadMenuTrigger);
